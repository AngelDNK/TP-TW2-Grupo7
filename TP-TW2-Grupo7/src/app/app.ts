...
@Component({
  ...
  template: `
        ...
        <div *ngIf="isMenuOpen" class="md:hidden border-t border-slate-700">
            ...
            <div class="px-2 pb-2 space-y-2">
              ...
              <a *ngIf="esCliente$ | async"
                ...
                <span class="relative">
                  ...
                  <span *ngIf="(totalItemsCarrito$ | async) as total"
                        class="absolute -top-2 -right-3 bg-red-600 text-white text-xs font-bold rounded-full h-4 w-4 flex items-center justify-center">
                    {{ total }}
                  </span>
                </span>
              </a>
          ...
          <ng-template #noLogueadoMobile>
            <div class="px-2 pt-2 pb-3 space-y-2">
              <a routerLink="/signin" (click)="closeMenu()"
    ...
    <footer class="bg-slate-800 text-gray-300 text-center py-4 mt-6">
      BARREGO, LEANDRO - DANGELO, AGUSTIN - DIAZ, DAMIAN - DI ROCCO DEL GIUDICE, GIULIANA - LEYES, ANGEL - PIEDRAFITA, SANTIAGO
    </footer>
  `,
  styles: [
    `
      .nav-link { margin-left: 20px; cursor: pointer; text-decoration: none; }
